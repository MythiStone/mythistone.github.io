/*
 * loadout-view.js: shared gear + talent rendering (window.MythiLoadout).
 *
 * Draws a gear set in the spec page's double-column armory layout and decodes a
 * Blizzard loadout export string into the same positioned talent tree the spec page
 * and analyzer draw. Used by the tierlist gear modal (tierlist-modal.js) and the
 * streamer pages (streamer.js). A gear set is {slots: {HEAD: {id, name, icon,
 * quality, bonus[], enchant?, gems?[]}, ...}} and `talents` is the export string.
 * Enchant/gem icons come from gem_enchant_index.json, trees from
 * /assets/json/talent_trees/<spec>.json, internal item links from items_index.json.
 *
 * The Blizzard "serialization version 2" decode mirrors analyzer.js decodeLoadout
 * (same bitstream), kept separate so these pages need none of the analyzer's
 * meta-comparison machinery.
 */
(function () {
  "use strict";
  ...
  // Gear beside talents, the layout both the tierlist modal and streamer pages use.
  // Returns a promise so a caller can react once the catalogs are loaded.
  function render(el, loadout) {
    el.innerHTML = '<p class="text-sm text-secondary mb-0">Loading gear and talents…</p>';
    return Promise.all([loadItemsIndex(), loadGemEnchantIndex(), loadTree(loadout.specId)])
      .then(function (out) {
        el.innerHTML =
          '<div class="row g-4">' +
            '<div class="col-12 col-xl-5">' +
              '<h6 class="mb-2">Gear</h6>' + gearHtml(loadout, loadout.specId) +
            '</div>' +
            '<div class="col-12 col-xl-7">' +
              '<h6 class="mb-2">Talents</h6>' + talentsHtml(loadout.talents, out[2]) +
            '</div>' +
          '</div>';
        refreshDynamic(el);
      })
      .catch(function () {
        el.innerHTML = '<div class="alert alert-warning text-dark text-sm mb-0">' +
          '<i class="material-symbols-rounded align-middle me-1">warning</i>' +
          'Gear and talent details are unavailable right now.</div>';
      });
  }

  window.MythiLoadout = { render: render, esc: esc };
})();
