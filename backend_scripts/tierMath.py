"""Tier scoring and clustering shared by the index page and the tierlist
social images: exponentially level-weighted upgrade score with a 95% CI lower
bound (lb_ci), optimal 1-D k-means (ckmeans) tier binning and deterministic
repair so every S-F tier is non-empty. Pure stdlib - safe to import from any
renderer (no DB, no env vars, no side effects).

Extracted verbatim from generateIndexPage.py; that module (and its CI job)
keeps using these via `from tierMath import ...`.
"""
